				  ...
				  <div class="contact-form">
				  	<center><h2>Complaints</h2></center>
					    ...
					    <form name="contactus" method="post">
					    		 <?php
                                 $sql=mysqli_query($con,"select * from doctors where id='".$_SESSION['did']."'");
                                 $data=mysqli_fetch_array($sql);
                                //  echo "the session id is: ".$data['id'];
                                 ?>
					    	<div>
						    	<span><label>NAME</label></span>
						    	<span><input type="text" name="fullname" required="true" value="Dr.<?php echo htmlentities($data['doctorName']); ?>"></span>
						    </div>
						    <div>
						    	<span><label>E-MAIL</label></span>
						    	<span><input type="email" name="emailid" readonly="readonly" value="<?php echo htmlentities($data['docEmail']); ?>"></span>
						    </div>
						    <div>
