<!-- No problem with this piece of code as of now on 19/04/2020. pls store this piece in a safe vault as it needs no editing in the mean time -->
...
$sql1=mysqli_query($con,"update users set medhist='$medhis' where email='$patemail'"); // getting affected at the user side also in real time. only to be used as a one time utility. after editing is done from the edit patient.php codebase
$sql2 = mysqli_query($con,"select * from users where email='$patemail'");
$record = mysqli_fetch_array($sql2);
$patcontact = $record['contactno'];
$patname=$record['fullName'];
$gender=$record['gender'];
$pataddress=$record['address'];
$patage=$record['age'];


$sql3=mysqli_query($con,"insert into tblpatient(Docid,PatientName,PatientContno,PatientEmail,PatientGender,PatientAdd,PatientAge,PatientMedhis) values('$docid','$patname','$patcontact','$patemail','$gender','$pataddress','$patage','$medhis')");
